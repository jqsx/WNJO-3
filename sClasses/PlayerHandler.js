...
export default class PlayerHandler {
    ...
    #updatePlayer(player, message) {
        ...
        if (Math.abs(player.position.x - position.x) < 5 || true) player.position.x = position.x;
        if (Math.abs(player.position.y - position.y) < 5 || true) player.position.y = position.y;
    }
}